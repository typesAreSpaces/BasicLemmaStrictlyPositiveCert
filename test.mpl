...
#g3 := (x-2)*(x-1);
#g4 := -(x - 3);
##test(g1*g2*g4, g3, [g1*g2*g4, g3], x);
##test(g1*g4, g2*g3, [g1*g2*g4, g3], x);
#test(g1, g2*g4, [g1*g2*g4], x);
#test(g1*g2, g4, [g1*g2*g4], x);
#test(g1*g4, g2, [g1*g2*g4], x);
...
#gi := (x+1)*(x+3/4);
#gj := (x+1/4)*(x-1/2);
#test(h1*gi*h2, gj, [h1*gi*gj*h2], x);
#test(gi*gj, h1*h2, [h1*gi*gj*h2], x);
...
#alpha := 1/6*(13 - 215/(11827 + 36*(115599)^(1/2))^(1/3) + (11827 + 36*(115599)^(1/2))^(1/3));
#f := (x+1)^2*(x+1/2)*(x-1);
#compactified := -(x+alpha)*(x-5)*f;

#test((x+1/2)*(x-1), -(x+alpha)*(x-5)*(x+1)^2, [compactified], x, "??");

## Approach 2
#test((x+1/2)*(x-1), (x+1)^2, [g1], x, "??");
#test((x+1)^2, (x+1/2)*(x-1), [g1], x, "??");

# Certificate of nonnegative factor
#test((x+1)^3*(x-1), -(x+2)*(x-2), [g1], x, "??");

#test(x+1, -x, [-(x+1)*x], x, "??");
#test(x^2, x+1, [-(x+1)*x*(x-1)^2], x, "??");

#test(x+1, -x*(x-1)^2, [-(x+1)*x*(x-1)^2], x, "??");

#test((x+2)^2, (x+1)*x, [-(x+2)^2*x*(x-2)], x, "??");

#g1:= -(x + 2)*(x + 1)^3*(x - 1)*(x - 2);
#f := (x-1/2)*(x-1);
#test((x+1)^2, f, [g1], x, "??");

#g := -(x + 2)*(x + 1)^3*x^2*(x - 1)*(x - 2);
#f := (x-1/2)*(x-1);
#test(x^2, f, [g], x, "??");
...
#f1 := (x + 1)*x^2*(x - 1);
##f2 := -(3 + x)*(-3 + x)*(-(1/2) + x)^2*(1/4 + x)*(3/4 + x);
##f2 := -(3 + x)*(-3 + x)*(-(1/2) + x)^2;
#f2 := (-(1/2) + x)^2;
#HfG := [
    #x+3,
...
#test((x+1)^2, x*(x-1), [g1], x, "??");
#test((x-1)^2, (x+1)*x, [g1], x, "??");

#test((x+1)^3*(x-1), -(x+2)*(x-2), [g1], x, "??");

#HfG := [x + 3, (x + 1)*x, (x + 1/2)*(x - 1/2), x*(x - 1), -(x - 3)];
#g := -(x+3)*(x+1)*x^2*(x-1)*(x-3);

#test((x+1)*(x-1), -(x+3)*x^2*(x-3), HfG, x, "??");
#test((x+1)*(x-1), x^2, HfG, x, "??");

#g1 := x+3;
#g3 := (x+1/2)*(x-1/2);
#g5 := -(x-3);
#test(g1*g5, g3, [g1*g3*g5], x, "??");
#test(g1*g3, g5, [g1*g3*g5], x, "??");

#test(-(x+6)*(x-6), (x+3)*(x-3), [-(x+6)*(x+3)*(x-3)*(x-6)], x, "??");
test(-(41*x^2 + 79)/216*(x+6)*(x-6), (x+3)*(x-3), [-(41*x^2 + 79)/216*(x+6)*(x+3)*(x-3)*(x-6)], x, "??");
